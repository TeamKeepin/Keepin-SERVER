...
const createReminder = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (isAlarm == true) {
        if (daysAgo === '0' || daysAgo == '1' || daysAgo == '2' || daysAgo == '3' || daysAgo == '7') {
            ...
            var realDate = (0, moment_1.default)(customDate).subtract(daysAgo, 'd').format('YYYY-MM-DD');
        }
        else {
            res.status(returnCode_1.default.BAD_REQUEST).json({
                status: returnCode_1.default.BAD_REQUEST,
    ...
    var today = (0, moment_1.default)().format('YYYY-MM-DD');
    if (date < today) {
        var ispassed = true;
    }
    else {
    ...
    try {
        ...
        if (isAlarm == false) {
            ...
            result = yield services_1.reminderService.saveReminder({
                title,
                date,
                sendDate: realDate,
                isAlarm,
 ...
 */
// 리마인더 연도별 목록 조회
const getYearReminder = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const userId = req._id;
    ...
    try {
        const resultArray = yield services_1.reminderService.findYearReminder({ userIdx: userId, year: year });
        var reminders = [];
        var janArray = [];
        var febArray = [];
        var marArray = [];
        ...
        return res.status(returnCode_1.default.OK).json({ status: returnCode_1.default.OK, message: '연도별 목록 조회 성공', data });
    }
    catch (err) {
        console.error(err.message);
        res.status(returnCode_1.default.INTERNAL_SERVER_ERROR).json({
 ...
 * }
 */
const modifyReminder = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const userId = req._id;
    const reminderId = req.params.reminderId;
    const errors = (0, express_validator_1.validationResult)(req);
    if (!errors.isEmpty()) {
    ...
    try {
        ...
        // alarm 받을 거면, daysAgo 값이 있음.
        result = yield services_1.reminderService.modifyReminderWithDaysAgo({
            reminderId,
            title,
            date,
            sendDate: realDate,
        ...
        return res.status(returnCode_1.default.OK).json({ status: returnCode_1.default.OK, message: '리마인더 수정 성공' });
    }
    catch (err) {
        console.error(err.message);
        res.status(returnCode_1.default.INTERNAL_SERVER_ERROR).json({
 ...
 *
 */
const modifyReminderAlarm = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const userId = req._id;
    const reminderId = req.params.reminderId;
    const errors = (0, express_validator_1.validationResult)(req);
    if (!errors.isEmpty()) {
    ...
    try {
        ...
        if (isAlarm == true) {
            ...
            reminderDetail = yield services_1.reminderService.findDetailReminder({
            ...
            resultAlarm = yield services_1.reminderService.modifyReminderAlarm({
                reminderId,
                isAlarm,
                sendDate: realDate,
                daysAgo,
        ...
        return res.status(returnCode_1.default.OK).json({ status: returnCode_1.default.OK, message: '리마인더 알람 수정 성공' });
    }
    catch (err) {
        console.error(err.message);
        res.status(returnCode_1.default.INTERNAL_SERVER_ERROR).json({
...
//SendDate 조회
const getSendDate = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const today = (0, moment_1.default)().format('YYYY-MM-DD');
    const errors = (0, express_validator_1.validationResult)(req);
    if (!errors.isEmpty()) {
        res.status(returnCode_1.default.BAD_REQUEST).json({
    ...
    try {
        const resultArray = yield services_1.reminderService.findAlarmReminder({ today });
        return res.status(returnCode_1.default.OK).json(resultArray);
    }
    catch (err) {
        console.error(err.message);
        res.status(returnCode_1.default.INTERNAL_SERVER_ERROR).json({
